...
import * as extractedTailwindConfigStyle from '../../../../styles/src/core/tailwind-config.scss';
...
export class MskTailwindService {
  ...
  constructor() {
    ...
    const regexpForClass = /\.msk-tailwind-extracted-config\s\{([\s\S]*)\}/g;
    const style =
      regexpForClass.exec(extractedTailwindConfigStyle.default)?.[1].trim() ??
      '';
