...
export class MessageService {
  ...
  create(message: Message): Observable<Message> {
    return this.messages$.pipe(
      take(1),
      switchMap((messages) =>
        this._httpClient.post<Message>('assets/api/messages', { message }).pipe(
          map((newMessage) => {
  ...
  update(id: string, message: Message): Observable<Message> {
    return this.messages$.pipe(
      ...
      switchMap((messages) =>
        this._httpClient
          .patch<Message>('assets/api/messages', {
            id,
            message,
          })
          .pipe(
  ...
  delete(id: string): Observable<boolean> {
    return this.messages$.pipe(
      ...
      switchMap((messages) =>
        this._httpClient
          .delete<boolean>('assets/api/messages', { params: { id } })
          .pipe(
  ...
  markAllAsRead(): Observable<boolean> {
    return this.messages$.pipe(
      ...
      switchMap((messages) =>
        this._httpClient
          .get<boolean>('assets/api/messages/mark-all-as-read')
          .pipe(
